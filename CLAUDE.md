...
### Forcing an account's balance to match Starling (`src/balance.ts`) — currently disabled in favor of a call verified not to work

**Current active code path (`index.ts`'s `/backfill`) calls `setAccountBalance()`, i.e. `updateAccount(id, {balance_current})`, per explicit instruction to use it "despite the docs."** This is expected — by design — to have no effect. Verified against source: the real underlying `updateAccount` implementation only ever writes `{id, name, last_reconciled}`; anything else, `balance_current` included, is silently discarded, not an error. For a normal (non-bank-synced) account, balance isn't stored at all; `getAccounts()`/`getAccountBalance()` compute it live as `SUM(amount)` over the account's transactions. The only direct DB write to `balance_current` (`updateAccountBalance()`) isn't exposed by `@actual-app/api` at all — it's reserved for bank-sync-linked accounts (GoCardless/SimpleFin) caching the bank's own reported figure separately from the local ledger, which doesn't apply to accounts this project creates.

**`reconcileAccountBalance()` in `src/balance.ts` is the mechanism confirmed to actually work**, but its call site in `/backfill` is commented out, not deleted — re-enable by swapping `setAccountBalance(actualAccountId, targetBalance)` back for the commented-out `reconcileAccountBalance(actualAccountId, targetBalance, adjustmentDateStr)` line. It achieves "set the balance" the only way that actually works: a single synthetic **adjustment transaction** for the delta between Actual's computed balance and Starling's real one, dated the day before the backfill's `from` cutoff. This exists because backfilling from a cutoff necessarily excludes everything before it — an account with real history predating that cutoff would otherwise show a balance of just the imported transactions (often confusingly negative), not the account's actual balance.
